...
DHT11_DATA_TypeDef DHT11_DATA ; // them vao;
/*!
 * User application data buffer size
 */
...
static void PrepareTxFrame(uint8_t port) {
	...
	if (Read_TempAndHumidity ( & DHT11_DATA)==SUCCESS) {
				AppData[0] = 0x04;
				AppData[1] = 0x75;
		temperate1=DHT11_DATA.temp_int;
			AppData[2] = ((temperate1 / 10) >> 8) & 0xFF;
		  AppData[3] = (temperate1 / 10) & 0xFF;
			AppData[4] =0x06;
			AppData[4] =0x05;
			RH=DHT11_DATA.humi_int;
			AppData[6] = ((RH / 10) >> 8) & 0xFF;
		  AppData[7] = (RH / 10) & 0xFF;
			AppDataSize = 8;
			// HIEN THI GIA TRI

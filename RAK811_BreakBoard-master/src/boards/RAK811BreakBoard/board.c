...
void BoardInitPeriph( void )
{
    ...
    GpioInit( &Led2, LED_2, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 0 );
		GpioInit( &pinA8, pin8, PIN_INPUT, PIN_PUSH_PULL, PIN_PULL_UP, 0 );

		// to do
    GpioWrite( &Led1, 1 );
    GpioWrite( &Led2, 1 );
		//GpioWrite( &pinA8, 1 ); // waiting
    //Init GPS
    GpsInit( );
		DHT11_Init();
...
void BoardInitMcu( void )
{
    ...
    GpioInit( &SX1276.Xtal, RADIO_XTAL_EN, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
		// to do
		//GpioInit( &SX1276.Xtal, LED_1, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
		//GpioInit( &SX1276.Xtal, LED_2, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
		//
    if( McuInitialized == false )
    {
        McuInitialized = true;
